...
export function buildPDF(data, dataCallback, endCallback) {
  ...
  const costosYExtras = [
    [
      "Gastos Origen",
      formatearMoneda(data.gastos_origen),
      "Unificación Factura",
      formatearMoneda(data.unif_factura),
    ],
    ["Tarifa", formatearMoneda(data.tarifa), "TXL", formatearMoneda(data.txl)],
    [
      "Serv. Administrativo",
      formatearMoneda(data.serv_admin),
      "Seguro",
      formatearMoneda(data.seguro),
    ],
...
function agregarTablaCombinada(
  doc,
  tituloIzquierdo,
  tituloDerecho,
  filas,
...
function agregarPieDePagina(doc) {
  doc
    ...
    .fontSize(8)
    .text(
      "Contacto: info@megaship.com | Teléfono: 2903 9713",
      50,
      doc.page.height - 40,
      { align: "center" }
    );
...
export async function createTable(tableName, id, body) {
  try {
    ...
    console.error(
      `Error inesperado al crear registro en la tabla ${tableName}:`,
      err
    );
...
export async function getDataByEmail(tableName, email) {
  try {
    ...
    if (error) {
      console.error(
        `Error fetching data from ${tableName} with Email ${email}:`,
        error
      );
...
export async function updateData(id, body, tableName) {
  try {
    ...
    if (error) {
      console.error(
        `Error updating data in ${tableName} with ID ${id}:`,
        error
      );
      return null;
    }
...
export async function getTrabajo() {
  try {
    const { data, error } = await supabase
      .from("Trabajo")
      .select(
        " id, cliente(id, name, empresa), origen(id, name), destino(id, name), costo, validez_ini, validez_fin, medio, incoterm(id, name), progreso, un, Carga(id, Contenedores, cantidad, peso, Volumen, kg), servicio(name), tarifa, gastos_origen, handling, unif_factura, deposito, deposito_local(name), seguro, tlx, serv_admin, tt, costo_total, frecuencia, Vendedor(name), codigoVendedor"
      );
    if (error) {
