...
if (!supabaseKey) {
  console.error(
    "Error: SUPABASE_KEY no está configurada en las variables de entorno."
  );
...
app.get("/incoterms", async (req, res) => {
  try {
    ...
    if (!data || data.length === 0) {
      return res
        .status(404)
        .json({ error: "No se encontraron registros en incoterms." });
    }
...
app.get("/clientes", async (req, res) => {
  try {
    ...
    if (!data || data.length === 0) {
      return res
        .status(404)
        .json({ error: "No se encontraron registros en clientes." });
    }
...
app.post("/variables/:clase", async (req, res) => {
  try {
    ...
    if (!id) {
      return res
        .status(500)
        .json({ error: "Error al crear el registro común." });
    }
    ...
    if (!result) {
      return res
        .status(500)
        .json({ error: `Error al crear el registro en la tabla ${clase}.` });
    }
...
app.get("/deposito", async (req, res) => {
  try {
    ...
    if (!data || data.length === 0) {
      return res
        .status(404)
        .json({ error: "No se encontraron registros en depósito." });
    }
...
app.post("/trabajos", async (req, res) => {
  ...
  try {
    ...
    if (!id) {
      return res
        .status(500)
        .json({ error: "No se pudo crear el registro en Common." });
    }
...
app.get("/variables", async (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({ error: "Error interno del servidor al obtener variables." });
  }
});
...
app.patch("/vendedor/:id", async (req, res) => {
  ...
  try {
    ...
    res
      .status(500)
      .json({ error: "Error interno del servidor al actualizar vendedor." });
  }
});
...
app.post("/variables", async (req, res) => {
  ...
  try {
    ...
    res
      .status(500)
      .json({ error: "Error interno del servidor al crear variable." });
  }
});
...
app.patch("/variables/:id", async (req, res) => {
  ...
  try {
    ...
    res
      .status(500)
      .json({ error: "Error interno del servidor al actualizar variable." });
  }
});
...
app.patch("/vendedor/password/:id", async (req, res) => {
  ...
  if (!password || password.trim() === "") {
    return res
      .status(400)
      .json({ error: "La contraseña es requerida y no puede estar vacía." });
  }
...
app.get("/vendedor", async (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({ error: "Error interno del servidor al obtener vendedores." });
  }
});
...
app.post("/vendedor", async (req, res) => {
  ...
  try {
    ...
    res
      .status(500)
      .json({ error: "Error interno del servidor al crear vendedor." });
  }
});
...
app.patch("/vendedor/:id", async (req, res) => {
  ...
  try {
    ...
    if (!data || Object.keys(data).length === 0) {
      return res
        .status(404)
        .json({ error: "Vendedor no encontrado o no actualizado." });
    }
...
app.patch("/vendedor/estado/:id", async (req, res) => {
  ...
  try {
    ...
    res.status(500).json({
      error: "Error interno del servidor al actualizar el estado del vendedor.",
    });
  }
});
